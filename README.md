...
The runbook is five nodes, and the third one is gated:

```
validate-preconditions ──▶ backup ──▶ migrate [REQUIRES APPROVAL] ──▶ verify ──▶ switch
```

The agent runs the first two nodes. Validate passes. The backup lands. Then execution **stops** at `migrate` — the destructive step — because it is marked `requires_approval = true`.

While it is paused, the database is provably untouched. No `ALTER TABLE` has happened. That is not a warning. It is a stop.

A human looks at the pending step and says yes. Execution resumes. The migration lands. Verify confirms the column. Switch records it. Every step — including the pause and the approval — is in a signed audit envelope.
...
1. **The stop is real.** `requires_approval` pauses the runbook at the destructive step; the database is untouched while paused (verified with live queries, not claims).
2. **Execution is deterministic and sequential.** Steps run in order — validate before backup, backup before migrate. The migration cannot race ahead of its own backup.
