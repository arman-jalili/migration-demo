...
# Fails unless: (1) customers table exists, (2) email column is ABSENT.
...
if pg -tAc "SELECT column_name FROM information_schema.columns WHERE table_name='customers' AND column_name='email'" | grep -q email; then
  echo "FAIL: email column already exists — nothing to migrate"
  exit 1
fi
echo "OK: customers table present, email column absent (preconditions hold)"
